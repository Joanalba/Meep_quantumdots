...
This is the simulation for the setup varying the angle of incidence of the laser 
from 0 to 5 degrees. We obtain the FOM for every 0.5 degrees.
...
s = 12 #size of the cell - for this experiment in 2D it needs this size to have
       # reasonables results
...
laser_ang= np.linspace(0,5,n) #Angle for the laser
dipole_det = 0 #displacement of the quantum dot

#Different variables
eta = np.zeros(len(laser_ang))
beta2 = np.zeros(len(laser_ang))
FOM = np.zeros(len(laser_ang))
beta2_mod = np.zeros(len(laser_ang))
FOM_mod = np.zeros(len(laser_ang))
list_flux_laser_no_wvg = np.zeros(len(laser_ang))
list_flux_laser_wvg = np.zeros(len(laser_ang))

gamma_tot = QD_no_wvg(dipole_det,time)/(2*math.pi*freq)
beta1 = QD_wvg(dipole_det,time)

for j in range(len(laser_ang)):
    laser_no_wvg = LaserBeam_no_wvg(dipole_det,time,laser_ang[j])
    list_flux_laser_no_wvg[j] = laser_no_wvg
    laser_wvg = LaserBeam_wvg(dipole_det,time,laser_ang[j])
    eta[j] = laser_wvg[1]/laser_no_wvg
    list_flux_laser_wvg[j] = laser_wvg[1]
    beta2[j] = (laser_wvg[0]*(2*math.pi))/(gamma_tot)
    if j != 0:
        beta2_mod[j] = (laser_wvg[0]*(2*math.pi))/(gamma_tot*beta2_mod[0])
    else:
        beta2_mod[j] = (laser_wvg[0]*(2*math.pi))/(gamma_tot)
    FOM[j] = eta[j]/(beta1*beta2[j])
    FOM_mod[j] = eta[j]/(beta1*beta2_mod[j])
    print('')
    print('###')
    print(j)
    print('###')
...
a = FOM_mod[0]*beta2_mod[0]
FOM_mod[0] = a
beta2_mod[0] = 1

#Plotting
plt.figure()
plt.plot(laser_ang, eta, 'go-')
plt.xlabel('Angle')
plt.ylabel('Eta')
plt.savefig('Eta1')

plt.figure()
plt.plot(laser_ang, beta2, 'ro-')
plt.xlabel('Angle')
plt.ylabel('Beta2')
plt.savefig('Beta21')

plt.figure()
plt.plot(laser_ang, FOM, 'mo-')
plt.xlabel('Angle')
plt.ylabel('FOM')
plt.savefig('FOM1')

plt.figure()
plt.plot(laser_ang, beta2_mod, 'ro-')
plt.xlabel('Angle')
plt.ylabel('Beta2')
plt.savefig('Beta21_mod')

plt.figure()
plt.plot(laser_ang, FOM_mod, 'mo-')
plt.xlabel('Angle')
plt.ylabel('FOM')
plt.savefig('FOM1_mod')


with open('FOM.txt','w') as f:
    for j in range(len(laser_ang)):
        f.write(str(laser_ang[j]))
        f.write('\t')
        f.write(str(FOM[j]))
        f.write('\n')
